...
size_t find_matches(s_output *output, size_t output_index, char *str, size_t str_index, size_t len) {
	size_t max_lookahead = LOOK_AHEAD;
	size_t max_search_buffer = SEARCH_BUFFER;

	/* PSEUDO_CODE
		if i - max_search_buffer < 0
			start_output = &output
		else:
			start_output = &[output - max_search_buffer]
		
		concat output[].c values into a string => str_output
		search_window = 0
	...
	*/
	...
	size_t concat_output_len = (size_t)(&output[output_index] - start_output);
	char *concat_output = (char *)malloc(sizeof(char) * concat_output_len + sizeof(char));
	concat_output[concat_output_len] = '\0';
	for (size_t i = 0; i < concat_output_len && &start_output[i] < &output[output_index]; i++) {
		concat_output[i] = start_output[i].c;
	}
	printf("%s ; concat_output_len=%d\n", concat_output, concat_output_len);
	...
	for (size_t lookahead = 1; lookahead <= max_lookahead && str_index + lookahead < len && start_output + lookahead <= &output[output_index]; ++lookahead) {
		// printf("[%c¥%s\n", concat_output[0], &str[str_index]);

		char *concat_output2 = strndup(concat_output, lookahead); // ugly code
		char *str2 = strndup(&str[str_index], lookahead);
		found_str = strnstr(concat_output2, str2, lookahead);

		// BUG 01234567890123456789HELLOHELLOHELLOHELLO%
		// printf("\t->%p", found_str);
		if (NULL == found_str)
			break;
		found_str_len = strlen(found_str);
		output_elem.c = str[str_index + 1];


		output_elem.back = concat_output_len - (size_t)(found_str - concat_output2); // debug but tired
		output_elem.size = lookahead;
		// printf("(%s)%c", found_str ? found_str : "", str[str_index]);
		// printf("%c %u %u", output_elem.c, output_elem.back, output_elem.size);
		// if (lookahead >= 2) { printf("FUCK FUCK FUCK \nFUCK\n±\n\n\tFUUUUUCJK");}
	}
	if (NULL == found_str_len) {
		printf("not found");
		output[output_index] = (s_output) { .c = str[str_index], .back = 0, .size = 0 };
		printf("%c", output[output_index].c);
	}
	else {
